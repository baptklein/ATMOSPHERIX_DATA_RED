...
def load_data(filename,select_ord,list_ord1,dire_mod):
    ...
    with open(filename,'rb') as ccfile:
        orders,W_data,I_data,T_obs,phase,window,berv,Vc,SN,proj,flux = pickle.load(ccfile)
    list_ord = []
    ...
    return list_ord,wl,data_tot,phase,window,Stdtot,SNRtot,projtot,fluxtot,F,Vc,berv
...
def perform_correlation(list_ord,data_tot,projtot,fluxtot,Stdtot,SNRtot,F2D,phase2,window2,Vstar,pos,Kp,Vsys,nbor,\
    ...
    fluxtot (list of ndarray): List of flux arrays.
    ...
    for no in range(len(list_ord)):
        ...
        for i in range(Nkp):
            for j in range(Nv):
                if use_proj:
                    if proj_fast:
                        interpmod = (1.-F2D[no](Kp[i]*np.sin(2.0*np.pi*np.array(phase2))+Vsys[j]+Vstar[pos]))*window2*fluxtot[no][pos]
