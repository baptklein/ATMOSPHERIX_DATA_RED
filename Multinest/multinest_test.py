...
parameters = ["Kp","Vsys","H2O","T_eq","CO","CO2","dr","Pcloud","vrot","haze"]
...
def prior(cube, ndim, nparams):
    cube[0] = pri.UniformPrior(cube[0], 120., 140.)           # uniform Kp 0:300
    cube[1] =  pri.UniformPrior(cube[1], -15.,-3.)           # un
    cube[2] =  pri.UniformPrior(cube[2], -4., -3.9)           # un
    cube[3] =  pri.UniformPrior(cube[3], 650.,700.)            # un
    cube[4] =  pri.UniformPrior(cube[4], -8.,-1.)
    cube[5] =  pri.UniformPrior(cube[5], -8.,-1.)
    cube[6] =  pri.UniformPrior(cube[6], 0.99,1.01)
    cube[7] = pri.UniformPrior(cube[7], 1.,2.)
    cube[8] = pri.UniformPrior(cube[8], 100.,101.)
    cube[9] = pri.UniformPrior(cube[9],0.99,1.0)


def loglike(cube, ndim, nparams):
    Kp, Vsys,H2O,T_eq,CO,CO2,dr,Pcloud,vrot,haze  = cube[0], cube[1], cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9]
    param_dic = dict(Kp=Kp, Vsys=Vsys,MMR_H2O=H2O,T_eq=T_eq,MMR_CO=CO,MMR_CO2=CO2,dr=dr,Pcloud=Pcloud,vrot=vrot,haze = haze)
    ...
    loglikelihood_LR = like_LR.calc_likelihood_HR( model_dic, data_dic)
...
pymultinest.run(loglike, prior, n_params,outputfiles_basename = 
                "/home/fdebras/Multinest_from_calmip/Multinest_WASP127/medianout_zoom-CO-CO2-radius_guillot_vrot/", 
